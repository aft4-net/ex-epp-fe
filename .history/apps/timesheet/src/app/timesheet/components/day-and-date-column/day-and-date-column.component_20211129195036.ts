import { Component, Input, OnInit, Output, EventEmitter, OnChanges, AfterViewInit, Directive, ElementRef, QueryList, ViewChildren, TemplateRef, ViewChild, ContentChildren } from '@angular/core';
...
export class DayAndDateColumnComponent implements OnInit, OnChanges,AfterViewInit {
  ...
  //@ViewChildren('entries') entries!: QueryList<TemplateRef<DayAndDateColumnComponent>>;
   @ContentChildren('entries')  entriesDiv!: QueryList<ElementRef>;
  ...
  ngOnChanges(): void {
    ...
    //     this.overflowPt=this.entriesDiv.get(findIndex(x=>x.getBoundingClientRect().bottom < document.getElementById('col')!.getBoundingClientRect  );
    //   }
    // }
    // }
    this.entriesDiv.changes.subscribe(() => {
      this.entriesDiv.toArray().forEach(el => {
          console.log(el);
          console.log(el.nativeElement.getBoundingClientRect().bottom)
   if(el.nativeElement.getBoundingClientRect.bottom< this.pointerEl.nativeElement.getBoundingClientRect().top){
              this.overflowPt=this.index;
                    this.index!++;
  ...
  //console.log(this.pointerEl.nativeElement.getBoundingClientRect().top)
  }
