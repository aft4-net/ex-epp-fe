import { Component, Input, OnInit, Output, EventEmitter, OnChanges, AfterViewInit, Directive, ElementRef, QueryList, ViewChildren, TemplateRef } from '@angular/core';
...
export class DayAndDateColumnComponent implements OnInit, OnChanges,AfterViewInit {
  ...
  entries!: QueryList<TemplateRef<DayAndDateColumnComponent>>;
  @ViewChildren('entries')
  ...
  ngAfterViewInit(): void {
    const list=this.entries.toArray();
  for (let i=0;i<=list.length;i++){
   let of=this.entriesDiv.filter(elt=>elt.nativeElement.parent.getBoundingClientRect.top==document.getElementById('btn')!.getBoundingClientRect().bottom);
  //this.overflowPt=this.entriesDiv.findIndex(elt=>elt.nativeElement.parent.getBoundingClientRect().bottom==document.getElementById('btn')!.getBoundingClientRect().top);
    this.split(this.overflowPt);
    console.log(of.findIndex)
