...
export class DayAndDateColumnComponent implements OnInit, OnChanges,AfterViewInit {
  ...
  ngOnChanges(): void {
    ...
    this.entriesDiv?.changes.subscribe(() => {
      ...
      i=>{i.nativeElement.getBoundingClientRect().bottom});
          console.log(el);
          // console.log(this.entriesDiv.nativeElement.children[0].getBoundingClientRect.bottom)  this.entriesDiv?.toArray().map(
