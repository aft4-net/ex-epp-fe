...
export class DayAndDateColumnComponent implements OnInit, OnChanges {
  ...
  checkOverflow (el: HTMLElement,index?: number) {if (el.offsetHeight < el.scrollHeight){
    this.overflow=true;
    }
    ...
    for (let i=0;i<=this.timeEntrys.length;i++){
      if(this.checkOverflow(column!,i)){
        console.log(this.checkOverflow(column!));
        this.moreEntries=new Array();
        for(let j=0;j<=i;j++){
        this.moreEntries[j]=this.timeEntrys[i];
      }
      console.log(this.moreEntries.length);
         }
    }
       }
    ...
    GetChildCount () {
      ...
      this.childCount = col!.childElementCount;
