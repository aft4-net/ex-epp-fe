...
constructor(public elRef:ElementRef){
  @Input() entry: any; // decorate the property with @Input()
}
}
@Component({
...
export class DayAndDateColumnComponent implements OnInit, OnChanges,AfterViewInit {
  ...
  for (let entry in this.entries)
    this.entriesDiv.filter(elt=>elt.nativeElement.getBoundingClientRect.bottom==document.getElementById('btn')!.getBoundingClientRect().top)
  }
