            ...
            steps{
                script {
                 ...
                 sshagent(credentials : ['staging']) {
                  ...
                  sh "rsync -rv --delete -e 'ssh' ./docker-compose.yml ubuntu@3.138.163.97:."  
                  
                  sh "ssh -o StrictHostKeyChecking=no  ubuntu@3.138.163.97 sudo docker-compose down"
                  sh "ssh -o StrictHostKeyChecking=no  ubuntu@3.138.163.97 sudo docker system prune -af"
                  sh "ssh -o StrictHostKeyChecking=no  ubuntu@3.138.163.97 sudo docker-compose up -d "
